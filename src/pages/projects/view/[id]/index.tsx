...
import { deleteTeamMemberProjectById } from 'apiSdk/team-member-projects';
...
function ProjectViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProjectInterface>(
    ...
    () =>
      getProjectById(id, {
        relations: ['agency', 'user', 'task', 'team_member_project'],
      }),
  );
  ...
  const team_member_projectHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteTeamMemberProjectById(id);
      await mutate();
    } catch (error) {
      setDeleteError(error);
    }
              ...
              {hasAccess('team_member_project', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/team-member-projects/create?project_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.team_member_project?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/team-member-projects/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/team-member-projects/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    team_member_projectHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
